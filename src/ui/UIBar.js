...
export default class UIBar extends UIElement {
    constructor({ parent, text, value, hint, id, min, max, step, changeFunc }) {
        super({ parent, type: 'div', id, className: 'UIBar' });
        this.text = text;
        this.value = value;
    ...
    _createTextElement(text, value, id) {
        const el = new UIElement({ parent: this.element, type: 'div', id: `${id}-text`, className: 'UIBar-text' });
        el.set('innerHTML', `${text}: ${value}`);
        return el;
    }

    _createInputElement(value, min, max, step, id) {
        const el = new UIElement({ parent: this.element, type: 'input', id: `${id}-input`, className: 'UIBar-input' });
        el.set('value', value)
            .set('type', 'range')
            .set('max', max)
            .set('min', min)
